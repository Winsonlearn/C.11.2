...
def edit(id):
    ...
    try:
        ...
        if not product_detail:
            flash("Detail produk tidak ditemukan!", "danger")
            return redirect(url_for('product_details.index'))
    except Exception as e:
        flash(f"Error: {str(e)}", "danger")
        return redirect(url_for('product_details.index'))
...
def delete(id):
    product_details_model = ProductDetailsModel()
    try:
        affected_rows = product_details_model.delete_product_details(id)
        if affected_rows > 0:
            flash("Detail produk berhasil dihapus!", "success")
        else:
            flash("Produk tidak ditemukan atau tidak dapat dihapus!", "warning")
    except Exception as e:
        flash(f"Error menghapus produk: {str(e)}", "danger")
    return redirect(url_for('product_details.index')) 